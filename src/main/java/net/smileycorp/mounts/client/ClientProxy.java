...
public class ClientProxy extends CommonProxy {
	...
	public void postInit(FMLPostInitializationEvent event) {
		...
		//add the leather horse armour layer to entities that can wear leather horse armour
		RenderManager renderManager = Minecraft.getMinecraft().getRenderManager();
		for (Map.Entry<Class<? extends Entity>, Render<? extends Entity>> entry : renderManager.entityRenderMap.entrySet()) {
			if (!IWearsHorseArmor.class.isAssignableFrom(entry.getKey())) continue;
			Render<?> renderer = entry.getValue();
			if (!(renderer instanceof RenderLiving)) continue;
			((RenderLiving) renderer).addLayer(new LayerLeatherHorseArmour<>((RenderLiving<? extends EntityLiving>) renderer, new ModelScalableHorse(0.1f)));
		}
	}
